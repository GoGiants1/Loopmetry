...
class CodexHistoryAdapter:
    ...
    def import_candidates(
        ...
        for candidate in candidates:
            ...
            previous_since = (
                None
                if rotated or not previous_position
                else _parse_iso(previous_position.get("since"))
            )
            previous_until = (
                None
                if rotated or not previous_position
                else _parse_iso(previous_position.get("until"))
            )
...
def _resume_index(
    ...
    if saved_hash is not None and saved_hash != _first_line_hash(path):
        key = (
            "checkpoint_reset",
            "rollout rotated or replaced; re-importing from the start",
        )
        diagnostic_counts[key] = diagnostic_counts.get(key, 0) + 1
        return 0
...
def _patch_target_paths(patch_text: str) -> list[tuple[str, str]]:
    ...
    action follows the canonical add/modify/delete vocabulary.
    """
    ...
    for match in _PATCH_FILE_RE.finditer(patch_text):
        ...
        if not raw_path:
            continue
        action = {"Add": "add", "Update": "modify", "Delete": "delete"}[verb]
    ...
    def finalize_stalled(self, *, previous_records_read: int) -> list[Event]:
        ...
        for call_id in list(self.pending):
            ...
            if file_did_not_grow and was_already_pending:
                ...
                # Without a completion record, an apply_patch call proves only
                # an attempted change. Emitting file_change here would invent
                # evidence that the patch was applied.
                if not entry.get("is_apply_patch"):
