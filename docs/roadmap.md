...
4. **Hybrid auto mode.** *(implemented)* `loopmetry run --source auto`: merge hook, explicit, and consented history evidence; deduplicate with provenance preserved; surface `adapter_conflict` diagnostics and a coverage summary. Interactive terminals preview history candidates and ask before import; non-interactive runs use hook and explicit input only unless `--include-history` is passed. Default backfill window is the assignment's configured start/end (decision D-012), not an unbounded or fixed-lookback default.
