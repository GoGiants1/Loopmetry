...
class ImportTests(unittest.TestCase):
    ...
    def test_apply_patch_delete_file_header_sets_delete_action(self) -> None:
        patch = "*** Begin Patch\n*** Delete File: src/old_file.py\n*** End Patch"
        run, _ = self._import([_local_shell_call("call-2e", ["apply_patch", patch])])
        changes = [e for e in run.events if e.type.value == "file_change"]
        self.assertEqual(len(changes), 1)
        self.assertEqual(changes[0].data, {"path": "src/old_file.py", "action": "delete"})
    ...
    def test_stalled_apply_patch_does_not_claim_a_file_change(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp) / "work" / "project"
            root.mkdir(parents=True)
            codex_home = Path(tmp) / "codex-home"
            ...
            _write_rollout(
                ...
                [
                    ...
                    _local_shell_call("call-stalled", ["apply_patch", patch], status="in_progress"),
                ],
            )
            adapter = CodexHistoryAdapter(codex_home=codex_home)
            context = _DC(project_root=root)
            candidates = adapter.discover(context)
            first = adapter.import_candidates(candidates, context)

            second = adapter.import_candidates(candidates, context, checkpoint=first.checkpoint)

            self.assertEqual(second.events, ())
            self.assertIn("stalled_tool_call", {diagnostic.kind for diagnostic in second.diagnostics})
            position = next(iter(second.checkpoint.positions.values()))
    ...
    def test_rotated_rollout_resets_checkpoint(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            path = _write_rollout(
                sessions_dir,
                "rollout-a.jsonl",
                [_session_meta(str(root), session_id="old"), _user_message("old")],
            )
            adapter = CodexHistoryAdapter(codex_home=codex_home)
            context = _DC(project_root=root)
            candidates = adapter.discover(context)
            ...
            path.write_text(
                json.dumps(_session_meta(str(root), session_id="new"))
                + "\n"
                + json.dumps(_user_message("replacement"))
                + "\n",
                encoding="utf-8",
            )
            rotated_candidates = adapter.discover(context)
            second = adapter.import_candidates(
                rotated_candidates, context, checkpoint=first.checkpoint
            )

            self.assertEqual(len(second.events), 1)
            self.assertIn("checkpoint_reset", {diagnostic.kind for diagnostic in second.diagnostics})
